...
withgui: cynkernel.bin libguitoolkit.so

CyanOS.iso: cynkernel.bin
	mkdir iso
	mkdir iso/boot
	mkdir iso/boot/grub
	# mkdir iso/system
	...
	echo 'menuentry "CyanOS" {' >> iso/boot/grub/grub.cfg
...
.PHONY: clean
clean:
	rm -rf obj cynkernel.bin libguitoolkit.so CyanOS.iso
